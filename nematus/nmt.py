...
def prepare_data_without_EOS(seqs_x, seqs_y, maxlen=None, n_words_src=30000,
                 n_words=30000):
    # x: a list of sentences
    lengths_x = [len(s) for s in seqs_x]
    lengths_y = [len(s) for s in seqs_y]
    ...
    if options['use_dropout']:
        ...
        if sampling:
            if options['model_version'] < 0.1:
                rec_dropout = theano.shared(numpy.array([retain_probability_hidden]*2, dtype=floatX))
                rec_dropout_r = theano.shared(numpy.array([retain_probability_hidden]*2, dtype=floatX))
                emb_dropout = theano.shared(numpy.array([retain_probability_emb]*2, dtype=floatX))
                emb_dropout_r = theano.shared(numpy.array([retain_probability_emb]*2, dtype=floatX))
                source_dropout = theano.shared(numpy_floatX(retain_probability_source))
            else:
                rec_dropout = theano.shared(numpy.array([1.]*2, dtype=floatX))
                rec_dropout_r = theano.shared(numpy.array([1.]*2, dtype=floatX))
                emb_dropout = theano.shared(numpy.array([1.]*2, dtype=floatX))
                emb_dropout_r = theano.shared(numpy.array([1.]*2, dtype=floatX))
                source_dropout = theano.shared(numpy_floatX(1.))
        else:
            if options['model_version'] < 0.1:
                scaled = False
            else:
...
def build_model(tparams, options):
    ...
    if options['use_dropout']:
        ...
        rec_dropout_d = theano.shared(numpy.array([1.]*5, dtype=floatX))
        emb_dropout_d = theano.shared(numpy.array([1.]*2, dtype=floatX))
        ctx_dropout_d = theano.shared(numpy.array([1.]*4, dtype=floatX))
    ...
    opt_ret["probs"] = probs
    # function sample from conditionals probability probs:    
    
    pick = trng.multinomial(pvals=probs).argmax(1)     
    rd = theano.function([x, x_mask, y, y_mask], outputs = pick)
    opt_ret["random_pick"] = rd #theano.function([x, x_mask, y, y_mask], outputs = probs)
    # cost
    y_flat = y.flatten()
...
def build_sampler(tparams, options, use_noise, trng, return_alignment=False):
    ...
    if options['use_dropout'] and options['model_version'] < 0.1:
        ...
        retain_probability_source = 1-options['dropout_source']
        retain_probability_target = 1-options['dropout_target']
        rec_dropout_d = theano.shared(numpy.array([retain_probability_hidden]*5, dtype=floatX))
        emb_dropout_d = theano.shared(numpy.array([retain_probability_emb]*2, dtype=floatX))
        ctx_dropout_d = theano.shared(numpy.array([retain_probability_hidden]*4, dtype=floatX))
        target_dropout = theano.shared(numpy_floatX(retain_probability_target))
    else:
        rec_dropout_d = theano.shared(numpy.array([1.]*5, dtype=floatX))
        emb_dropout_d = theano.shared(numpy.array([1.]*2, dtype=floatX))
        ctx_dropout_d = theano.shared(numpy.array([1.]*4, dtype=floatX))
    ...
    init_state = tensor.matrix('init_state', dtype=floatX)
...
def build_sampler_from_prefix(tparams, options, use_noise, trng, return_alignment=False):
...
def build_full_sampler(tparams, options, use_noise, trng, greedy=False):
    ...
    if options['use_dropout'] and options['model_version'] < 0.1:
        ...
        emb_dropout_d = theano.shared(numpy.array([1.]*2, dtype=floatX))
        ctx_dropout_d = theano.shared(numpy.array([1.]*4, dtype=floatX))
        target_dropout = theano.shared(numpy_floatX(1.))
...
def build_full_sampler_from_prefix(tparams, options, use_noise, trng, greedy=False):
    # initialize dropout
    if options['use_dropout'] and options['model_version'] < 0.1:
        retain_probability_emb = 1 - options['dropout_embedding']
        retain_probability_hidden = 1 - options['dropout_hidden']
    ...
    shared_vars = [tparams[pp('decoder', 'U')],
                   ...
                   tparams[pp('decoder', 'b_nl')],
                   tparams[pp('decoder', 'bx_nl')]]
    # compute t_th state corresponding to prefix of length t 
    def decoder_step_with_mold(y, init_state, ctx, pctx_, target_dropout, emb_dropout, rec_dropout, ctx_dropout, *shared_vars):
        ...
        next_state = tensor.switch(
                      tensor.eq(y[:,None],0),
                      init_state,
                      proj[0])
        return next_state
    
    prefix = tensor.matrix(name="prefix",dtype="int64")
    init_state_p, _ = theano.scan(decoder_step_with_mold,
                        outputs_info=[init_state], sequences = [prefix],
                        non_sequences=[ctx, pctx_, target_dropout, emb_dropout_d, rec_dropout_d, ctx_dropout_d]+shared_vars,
                        truncate_gradient=options['decoder_truncate_gradient'],)
                                                                                                  
    # Sampling from prefix:                                                         
    init_w = tensor.vector(name = "last_word", dtype = "int64")
    init_state_s = tensor.tile(init_state_p[-1], [1, 1])
    if not greedy:        
        k = tensor.iscalar("k")
        k.tag.test_value = 12                               
        
        ctx = tensor.tile(ctx, [k, 1])
        init_state_s = tensor.tile(init_state_p[-1], [k, 1])
    
    #f_shape = theano.function([prefix,x,k],init_state_s.shape,name='f_shape')
    ...
    (sample, state, probs), updates = theano.scan(decoder_step,
                        outputs_info=[init_w, init_state_s, None],
                        non_sequences=[ctx, pctx_, target_dropout, emb_dropout_d, rec_dropout_d, ctx_dropout_d]+shared_vars,
                        n_steps=n_steps, truncate_gradient=options['decoder_truncate_gradient'],)

    print >>sys.stderr, 'Building f_sample...',
    if greedy:
        inps = [x, x_mask, n_steps, prefix, init_w]
    else:
        inps = [x, x_mask, k, n_steps, prefix, init_w]
    outs = [sample, probs]
    f_sample = theano.function(inps, outs, name='f_sample', updates=updates, profile=profile)
...
def train(dim_word=512,  # word vector dimensionality
          ...
          datasets=[
              '/data/train10/train10.en.tok',
              '/data/train10/train10.fr.tok'],
          valid_datasets=['/data/test/test10/test10.en.tok',
                          '/data/test/test10/test10.fr.tok'],
          dictionaries=[
              '/data/train10/train10.en.tok.pkl',
              '/data/train10/train10.fr.tok.pkl'],
          use_dropout=False,
